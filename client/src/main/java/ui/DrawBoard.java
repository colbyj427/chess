...
public class DrawBoard {
  ...
  private static void drawChessBoard(PrintStream out, ChessPiece[][] pieceArray) {
    for (int boardRow=7; boardRow >= 0; --boardRow) {
      ...
      if (boardRow % 2 == 0) {
        drawWhiteRowOfSquaresBlackBottom(out, boardRowString, pieceArray, false);
      } else {
        drawBlackRowOfSquaresBlackBottom(out, boardRowString, pieceArray, false);
      }
    }
  }
  ...
  private static void drawChessBoardBlackBottom(PrintStream out, ChessPiece[][] pieceArray) {
    for (int boardRow=0; boardRow < BOARD_SIZE_IN_SQUARES; ++boardRow) {
      String boardRowString=Integer.toString(boardRow + 1);
      if (boardRow % 2 == 0) {
        drawWhiteRowOfSquaresBlackBottom(out, boardRowString, pieceArray, true);
      } else {
        drawBlackRowOfSquaresBlackBottom(out, boardRowString, pieceArray, true);
      }
    }
  }
  private static void drawWhiteRowOfSquaresBlackBottom(PrintStream out, String row, ChessPiece[][] pieceArray, boolean blackBottom) {
    ...
    if (blackBottom == true) {
      for (int boardCol=0; boardCol < BOARD_SIZE_IN_SQUARES; ++boardCol) { // draw actual squares, one at a time
        drawWhiteRow(out, row, pieceArray, boardCol);
      }
    }
    else {
  ...
  private static void drawBlackRowOfSquaresBlackBottom(PrintStream out, String row, ChessPiece[][] pieceArray, boolean blackBottom) {
    int intRow=Integer.valueOf(row);
    printRowNum(out, row);
    if (blackBottom == true) {
      for (int boardCol=0; boardCol < BOARD_SIZE_IN_SQUARES; ++boardCol) { // draw actual squares, one at a time
        drawBlackRow(out, row, pieceArray, boardCol);
      }
    }
    else {
      for (int boardCol=7; boardCol >= 0; --boardCol) { // draw actual squares, one at a time
  ...
  private static void drawWhiteRow(PrintStream out, String row, ChessPiece[][] pieceArray, int boardCol) {
    String pieceType = " ";
  ...
  private static void drawBlackRow(PrintStream out, String row, ChessPiece[][] pieceArray, int boardCol) {
    String pieceType = " ";
    String pieceColor = "WHITE";
    if (squares[(Integer.valueOf(row)-1)][boardCol] != null) {
      pieceType=getPieceType(row, boardCol, pieceArray);
  ...
  private static void drawWhiteRowOfSquares(PrintStream out, String row, ChessPiece[][] pieceArray) {
  ...
  private static void drawBlackRowOfSquares(PrintStream out, String row, ChessPiece[][] pieceArray) {
    int intRow=Integer.valueOf(row);
    printRowNum(out, row);
    for (int boardCol=0; boardCol < BOARD_SIZE_IN_SQUARES; ++boardCol) { // draw actual squares, one at a time
      if (boardCol % 2 == 1) {
